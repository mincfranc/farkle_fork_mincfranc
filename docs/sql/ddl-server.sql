...
(
    ...
    primary key (game_id, user_profile_id)
);
create table roll
(
...
alter table if exists game add constraint FKn7s1942q5h1109lb8houdtbai foreign key (winner_id) references user_profile;
alter table if exists game_player add constraint FK8so14tnd5mqdjqabugc0cycxu foreign key (game_id) references game;
alter table if exists game_player add constraint FKlrv63161ntxv6dolgmlv32tc1 foreign key (user_profile_id) references user_profile;
alter table if exists roll add constraint FKk1f68rwaqf0c5w6brh6c59rlr foreign key (turn_id) references turn;
...
alter table if exists turn add constraint FKi3rn9e2enjeyqhdv9kmsgo6xh foreign key (user_id) references user_profile;
